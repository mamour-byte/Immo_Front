...
function DashboardHeader({ isAdmin, user, section, onSectionChange }) {
  ...
  return (
    <header className="rounded-lg border border-border bg-white px-4 py-4 shadow-sm sm:px-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <Link
            to="/account"
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-white px-3 py-2 text-sm font-medium text-text-main transition-colors hover:bg-surface"
          >
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary-dark px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-primary-dark"
          >
            <Plus size={16} />
            Ajouter un bien
          </button>
        ...
        {tabs.map(([key, label]) => (
          <button
            ...
            className={[
              ...
              section === key ? "bg-primary-dark text-white" : "bg-surface text-text-main",
            ].join(" ")}
          >
            {label}
          </button>
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {stats.map((stat) => {
        ...
        return (
          <article key={stat.label} className="rounded-lg border border-border bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary-light text-primary">
                <Icon size={20} />
              </span>
            </div>
            <p className="mt-3 text-xs font-medium text-text-muted">{stat.hint}</p>
          </article>
        );
      })}
    </div>
      ...
      {actions.map((item) => (
        <div key={item.title} className="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-lg border border-border px-3 py-2 text-sm font-medium text-text-main transition-colors hover:bg-surface"
          >
            {item.action}
          </button>
        </div>
...
function CompactPropertyList({ items, loading }) {
  ...
  if (!items.length) return <p className="text-sm text-text-muted">Aucun bien à afficher.</p>;

  return (
    <div className="space-y-3">
      {items.slice(0, 5).map((item) => (
        <div key={item.id} className="flex items-center justify-between gap-4 rounded-lg border border-border px-3 py-3">
          <div className="min-w-0">
          ...
          <span className="shrink-0 rounded bg-surface px-2 py-1 text-xs font-medium text-text-main">
            {item.status || "-"}
          </span>
        </div>
      ))}
